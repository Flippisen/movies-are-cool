...
test('Selecting a new genre should reset current page', async () => {
    window.fetch = (window.fetch as jest.Mock).mockImplementation(() => {
        return {
            ok: true,
            status: 200,
    ...
    selectMaterialUiSelectOption(genreDropdown, 'test-genre2');

    await waitFor(() => expect(window.fetch).toHaveBeenCalledTimes(5));
    expect(window.fetch).toHaveBeenNthCalledWith(5,
        'https://api.themoviedb.org/3/discover/movie?api_key=e700250595abfd12d46b05e82480206c&with_genres=1,2&sort_by=popularity.desc&page=1',
        {
            method: 'GET',
            signal: undefined
        }
...
test('Selecting a new sort order should reset current page', async () => {
    window.fetch = (window.fetch as jest.Mock).mockImplementation(() => {
        return {
            ok: true,
            status: 200,
